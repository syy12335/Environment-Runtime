...
notebook 的 GRPO 节点会在训练结束后解析 `verl_stdout.log` 和 `reward_audit.jsonl`，写出 `grpo_step_metrics.jsonl`、`grpo_reward_audit_summary.json`、`grpo_diagnostics.json`，并直接渲染 score 曲线。
如果把 `run.predict_holdout` 打开，同一节点会按 `holdout_inference.max_samples` 抽样 holdout rollout；默认是 30 条，随后 `run.evaluate=true` 会调用 teacher 评测准确率并展示图表。
